...
namespace _22._08._15_homework
{
    internal class Program
    {
        ...
        static void WaitMenu(ref int posY) // 메인메뉴
        {
            ...
            do
            {
                for (int k = 0; k < 2; k++) //빈칸은 자동적으로 위치함
                {
                    SetCursorPosition(10, k + 9); // 빈칸
                    Write("  ");
                }
                SetCursorPosition(10, posY + 9); // posY 값에 따라 커서 위치 변동
                Write("▶"); // 커서 모양
                ...
                switch (key.Key)
                {
                    case ConsoleKey.UpArrow: // 위 입력
                        posY--; // Y 값이 감소하면서 위로 이동
                        if (posY < 0) // Y값이 계속 줄어들 경우 posY값을 강제적으로 바꿔주면서 탈출방지
                        {
                            posY = 1;
                        }
                        break;
                    case ConsoleKey.DownArrow: // 아래 입력
                        posY++; // Y 값이 증가하면서 아래로 이동
                        if (posY > 1) // Y값이 계속 늘어날 경우 posY값을 강제적으로 바꿔주면서 탈출방지
                        {
                            posY = 0;
                        }
                        break;
            ...
            SetCursorPosition(0, 0); //커서 위치 초기화
        }
        ...
        static void Main(string[] args) //메인메뉴 와 게임 연결
        {
            ...
            do
            {
                if (_isStart) // _isStart = false 이므로 처음 시작할때는 패스
                {
                    switch (posY)
                    {
                        case 0: 
                ...
                else
                {
                    ...
                    _isStart = true; // _isStart = true 로 바뀌면 재반복시 if 문 진행
                    Clear();
                }

            } while (!_isFinish); // ! 로 _isFinish = false 를 true로 반전 시켜서 반복
        }
    }
}
